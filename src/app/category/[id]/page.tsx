...
const CATEGORY_FAQS: Record<string, { q: string; a: string }[]> = {
  ...
  'ai-agents': [
    { q: 'What is an AI agent and how is it different from a chatbot?', a: 'A chatbot generates text in response to a prompt; an AI agent takes actions to complete a goal — navigating websites, calling APIs, running code, and chaining multiple steps without being prompted at each one. Agents are the layer above chat, designed to actually finish tasks rather than describe them.' },
    { q: 'What are the best AI agent tools in 2026?', a: 'Standouts in 2026 include Codex in Chrome (OpenAI\'s browser-driving agent for ChatGPT users), Mochi.js (Bun-native browser automation built for agent workflows), and ClawTick (managed cron scheduling for recurring agent runs). The category covers consumer-facing browser agents, developer libraries for building agents, and infrastructure for running them in production.' },
    { q: 'Are AI agents free to use?', a: 'It depends on the layer. Mochi.js is fully open-source and free. Codex in Chrome is included with a ChatGPT subscription. Infrastructure tools like ClawTick offer free tiers for low-volume use, with paid plans as agent runs scale. Building your own agent on top of an LLM API will incur token costs even if the framework is free.' },
    { q: 'What can AI agents actually do?', a: 'Modern AI agents can drive a browser to fill forms and scrape data, execute multi-step workflows across multiple apps, run on a schedule rather than waiting for a prompt, and complete research or admin tasks end-to-end. The practical ceiling has moved from "answer a question" to "do the work and send me the result".' },
    { q: 'Do I need to be a developer to use AI agents?', a: 'No — consumer-facing agents like Codex in Chrome work out of the box for anyone with a ChatGPT account. Developer-focused tools like Mochi.js and ClawTick assume coding ability and are aimed at building or operating agents at scale. The category includes options at both ends of the technical spectrum.' },
    { q: 'Are AI agents safe to give access to my accounts and data?', a: 'AI agents inherit the permissions you give them, so the safety question is really about scope. For browser agents, prefer separate profiles or sandboxed sessions rather than your main logged-in browser. For automation tools driving APIs, scope tokens narrowly. Agents can be reliable for read-only tasks today; high-stakes write operations still warrant human review at each step.' },
  ],
}
